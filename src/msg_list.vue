<template>
	<div>
		...
		<div v-show="!msgList_loading">
			...
			<div v-for="msg in msgList" v-bind:id="'message-id-'+msg.id">
					...
					<div class="col-sm-20 col-xs-16">
						<div class="row f-msg-toolbar-top">
							<div class="col-sm-12">
								...
								<a v-else="" v-bind:href="'#message-id-'+msg.id">{{msg.date}}</a>
							</div>
	...
	export default {
		...
		methods: {
			...
			msg_fetch: function() {
				...
				var url = Math.random()>.1 ? 'backend-emu/msg-list.json' : '';
				$.getJSON(url, {})
				.done((data)=>{
				...
				.fail(()=>{
					...
					setTimeout(this.msg_fetch, 1000);
				})
				.always(()=>{})
			},
			msg_add: function () {
				...
				var newMsg = getMsg({ 
					...
					text: editorInstance.fromBBCode(val)
				});
				...
				.fail(()=>{
						notie.alert('error', 'Рандомная (10%) неудача отправки. Cообщение темы скрыто.', 3)
						this.msgList.splice(this.msgList.indexOf(newMsg), 1) // удаляем сообщение
					})
				.always(()=>{
